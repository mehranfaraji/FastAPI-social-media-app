...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(
    oauth2.get_current_user), limit: int = 10, skip: int = 0,
    search: Optional[str] = ""):
    ...
    vote = db.query(models.Post, func.count(models.Vote.post_id).label("votes")).join(models.Vote, models.Vote.post_id == models.Post.id, isouter=True).group_by(
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(
    ...
    # conn.commit()
    ## post is actually text of a SQL query
    post_quey = db.query(models.Post).filter(models.Post.id == id)
    post = post_quey.first()
